...
type ExistingCertificate = Pick<
  Database['public']['Tables']['certificates']['Row'],
  'id' | 'pdf_url' | 'issued_at' | 'meta'
>
    ...
    if (existingCertificateData) {
      ...
      if (supabaseUrl && existingCertificateData.pdf_url) {
        try {
          ...
          console.warn(
            'Failed to verify existing certificate object:',
            storageCheckError
          )
        }
      }
      ...
      if (objectExists) {
        ...
        console.info(
          'Outdated or invalid certificate detected; regenerating.',
          {
            userId: user.id,
            moduleId,
      ...
      console.warn(
        'Existing certificate record found but file missing, regenerating.',
        {
          userId: user.id,
          moduleId,
          certificatePath: existingCertificateData.pdf_url,
